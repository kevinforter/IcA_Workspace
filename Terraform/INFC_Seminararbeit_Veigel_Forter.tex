...
\placeholderfig{Einstellung der VPC inklusive manueller CIDR-Einstellung 10.0.0.0/16}{vpc-overview}
\FloatBarrier

\subsection{Erstellung Subnet}
\placeholderfig{Öffentliches Subnet mit Subnet-CIDR-Block 10.0.1.0/24}{subnet-overview}
\FloatBarrier
\placeholderfig{Automatische Vergabe der öffentlichen IPv4-Adresse aktiviert}{auto-public-ipv4}
\FloatBarrier
...
\placeholderfig{Erstellung der Sicherheitsgruppe mit SSH und HTTP}{create-inbound-rules}
\FloatBarrier
...
\placeholderfig{Anpassung der zuvor erstellten Sicherheitsgruppe mit zusätzlichen Regeln für Docker}{edit-inbound-rules}
\FloatBarrier

% ============================================================================
\subsection{EC2-Instanzen (Nodes)}

\placeholderfig{Erstellung der ersten Instanz in AWS}{create-instance}
\FloatBarrier
\placeholderfig{AMI: Ubuntu (neueste Version).}{operating-system}
\FloatBarrier
\placeholderfig{\texttt{t2.micro} (oder vergleichbar) als Instance Type}{instance-type}
\FloatBarrier
\placeholderfig{Aktivierung von \textbf{Auto-assign Public IP}.}{networksettings}
\FloatBarrier
\placeholderfig{Einfügen des User-Data-Skripts}{userdata}
\FloatBarrier
...
\placeholderfig{EC2 Launch – Übersicht der Einstellungen}{ec2-launch}
\FloatBarrier
...
docker --version
\end{lstlisting}
\placeholderfig{SSH-Verbindung und Docker-Version}{ssh-docker}
\FloatBarrier
...
\placeholderfig{\texttt{docker swarm init} auf der ersten Instanz}{docker-swarm-init}
\FloatBarrier
\placeholderfig{\texttt{docker node ls} auf einer Instanz}{node-ls-1-instance}
\FloatBarrier
\placeholderfig{Hinzufügen von zwei Workern}{add-two-worker}
\FloatBarrier
\placeholderfig{Generieren des Manager-Tokens}{manager-token}
\FloatBarrier
\placeholderfig{Hinzufügen eines zweiten Managers}{second-manager}
\FloatBarrier

\placeholderfig{\texttt{docker node ls} mit 4 Knoten}{node-ls}
\FloatBarrier
...
\placeholderfig{Portainer-Installation über die Kommandozeile}{portainer-installation}
\FloatBarrier

\placeholderfig{Erstellung eines neuen Logins für Portainer}{new-login}
\FloatBarrier
\placeholderfig{Angezeigte Services nach der Installation von Portainer}{standard-services}
\FloatBarrier
...
\end{lstlisting}
\placeholderfig{Service mit 3 Replikas und Port 80 erstellen}{create-service}
\FloatBarrier
\placeholderfig{Die 3 Replikas erhalten jeweils einen eigenen Slot}{three-replicas}
\FloatBarrier
\placeholderfig{Die 3 Replikas werden auf die 4 verschiedenen Nodes in AWS verteilt}{distribution-3-replicas}
\FloatBarrier

\placeholderfig{Hello-World-Seite nach Deployment des Service}{hello-world}
\FloatBarrier
...
\placeholderfig{Die 10 Replikas werden nach dem Hochskalieren auf verschiedene Slots verteilt}{ten-replicas}
\FloatBarrier
\placeholderfig{Die 10 Replikas werden auf die 4 verschiedenen Nodes verteilt}{distribution-10-replicas}
\FloatBarrier

\placeholderfig{Hello-World-Seite wird nun von verschiedenen Containern bedient}{hello-world-2-containers}
\FloatBarrier

% ACHTUNG: Datei ohne Leerzeichen benennen (z. B. shutdown-node.png)
\placeholderfig{Node wird als \enquote{down} angezeigt, nachdem eine EC2 heruntergefahren wurde}{shutdown-instance}
\FloatBarrier

\placeholderfig{Services auf dem betroffenen Node wechselten erst nach \enquote{ready}, dann nach \enquote{shutdown}, neue Replikas wurden auf andere Nodes verteilt}{status-ready}
\FloatBarrier
\placeholderfig{Node down mit zwei Services; auf den restlichen drei Nodes laufen insgesamt 10 Services}{node-down-10-services}
\FloatBarrier

\placeholderfig{Nach dem Hochfahren wurden die übrigen Services wieder sauber auf alle 4 Nodes verteilt}{still-shutdown}
\FloatBarrier

\placeholderfig{Beim Scale-Down wurden die verbleibenden 3 Services erneut gleichmäßig verteilt}{scale-down}
\FloatBarrier
...
\end{lstlisting}
\placeholderfig{Download und Installtion von Minikube}{minikube-install}
\FloatBarrier
...
\end{lstlisting}
\placeholderfig{Download und Installation von Kubectl}{kubectl-install}
\FloatBarrier
...
kubectl get nodes
\end{lstlisting}
\placeholderfig{Start von Minikube mit einer einzelnen Node (2GB Ram da sonst alles gebraucht wird)}{minikube-start}
\FloatBarrier
\placeholderfig{Alle laufenden Notes}{minikube-get-nodes}
\FloatBarrier
...
\FloatBarrier
\placeholderfig{Anzeigen des erstellten Deployments}{get-deployments}
\FloatBarrier
...
\FloatBarrier
\placeholderfig{Anzeigen der erstellten Pods}{nginx-get-pods}
\FloatBarrier
...
\end{lstlisting}
\placeholderfig{Erstelltes Dopleyment auf Port 8080 exposen}{expose-deployment}
\FloatBarrier
\placeholderfig{Browser öffnen mit Zugriff auf Pod}{Reach-service}
\FloatBarrier
...
\end{lstlisting}
\placeholderfig{Interner ClusterIP-Service für das nginx-Deployment}{clusterip}
\FloatBarrier
...
\end{lstlisting}
\placeholderfig{Erfolgreicher Curl-Test des internen Nodeport-Service}{Nodeport-test}
\FloatBarrier
...
kubectl expose deployment nginx \
  ...
  --target-port=80
\end{lstlisting}
\placeholderfig{LoadBalancer-Service mit zugewiesenem externen Port (z.\,B. 31245)}{loadbalancer-created}
\FloatBarrier
